...
class MoStressNeuralNetwork:
    def __init__(self, modelOpts, dataset):
        ...
        self._xTrain, self._xTest, self._yTrain, self._yTest = OperateModel._getTensorData(dataset["features"], dataset["targets"], self.modelOpts["trainningDataTestSize"])
    ...
    def execute(self, modelName = "REGULARIZER-LSTM", optimizer = "rmsprop"):
        ...
        print("\nModel Fitted\n")
        print("\Saving Model\n")
        OperateModel.saveModel(self.model, join("models", "saved", f"{self._modelName}-{self._optimizerName.upper()}.h5"))
        print("\nModel Saved\n")
        print("\nLearning Curves\n")
